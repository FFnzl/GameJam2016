...
public class RoomGenerator : MonoBehaviour
{
	...
	void Awake()
	{
		oneEntryRooms = Resources.LoadAll<GameObject>("prefabs/rooms/O");
		twoEntryRoomsL = Resources.LoadAll<GameObject>("prefabs/rooms/L");
		twoEntryRoomsD = Resources.LoadAll<GameObject>("prefabs/rooms/I");
		threeEntryRooms = Resources.LoadAll<GameObject>("prefabs/rooms/T");
		fourEntryRooms = Resources.LoadAll<GameObject>("prefabs/rooms/X");
	}
	...
	private float DiagonalRotation(bool[,] grid, int x, int y)
	{
	...
	{
		float rotation = DiagonalRotation(grid, x, y) > 0 ? 0 : 90;
				...
				if (grid[x, y])
				{
					...
					switch (CountRooms(grid, x, y))
					{
						...
						case 2:
							...
							{
								rotation = DiagonalRotation(grid, x, y);
					...
					map[x, y] = o;
					if (o != null && !startRoom) GameObject.FindGameObjectWithTag("Stats").GetComponent<Stats>().RoomsTotal++;
				}
			}
		}
